...
- 상태관리 도구는 아닙니다.
- 사용자 로그인 정도, 애플리케이션 환경 설정, 테마 등등

## 10.1. Context API를 사용한 전역 상태관리 흐름 이해하기

- 최상위 컴포넌트에서 여러 컴포넌트를 거쳐 props로 원하는 상태와 함수를 전달했지만
- Context API를 사용하면 Context를 만들어서 원하는 값을 받아와 사용할 수 있다.
...
- Context의 value 에는 상태 값 뿐만아니라 함수를 전달해 줄 수 있다.
- src/contexts/colorContext.js
